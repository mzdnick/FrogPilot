...
class CurveSpeedController:
  ...
  def update_target(self, v_cruise, v_ego, sm):
    if (sm["carState"].leftBlinker or sm["carState"].rightBlinker or np.argmax(sm["modelV2"].meta.desireState) in (log.Desire.turnLeft, log.Desire.turnRight, log.Desire.laneChangeLeft, log.Desire.laneChangeRight)):
      self.target = None
      return v_cruise
